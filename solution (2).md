...
2) Счетчик с Increment()

В данном случае операции Increment() в разных потоках неотличимы друг от друга. Итак, пусть у нас есть набор возвращаемых значений от Get(), а так же знание о том,
какие вызовы Increment() и Get() конкурируют между собой и друг с другом, а какие нет. Будем строить историю.

Для начала расставим неконкурирующие вызовы Increment(). Мы можем их относить к любому потоку, потому что они неотличимы друг от друга.
Добавление конкурирующих между собой операций Increment() на эту историю может быть абсолютно произвольным.

Теперь нужно разобраться с Get(). Их просто нужно вставить так, чтобы слева от операции стояло ровно столько точек Increment(),
чему равно возвращаемое значение соответствующего Get().
